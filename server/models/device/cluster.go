...
func DelClusters(clusterid string) (err error) {
	...
	var s Storage
	if num, err := o.QueryTable(new(Storage)).Filter("clusterid", clusterid).All(&s); err == nil && num > 0 {
		s.Clusterid = uuid
...
func GetClusters() (res []ResCluster, err error) {
	...
	for _, c := range clus {
		var clu ResCluster
		clu.Devices, err = _device(c.Uuid)
		clu.Cid = c.Cid
		clu.Uuid = c.Uuid
		clu.Zoofs = c.Zoofs
		clu.Store = c.Store
...
func _device(uuid string) (devs map[string][]string, err error) {
	o := orm.NewOrm()
