...
// AddClient insert a new Client into database and returns
// last inserted Id on success.
func AddClient(m *Client) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}
...
// GetAllClient retrieves all Client matches certain condition. Returns empty list if
// no records exist
func GetAllClient(query map[string]string, fields []string, sortby []string, order []string,
...
// DeleteClient deletes Client by Id and returns error if
...
func DeleteClient(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Client{ExportInit{Id: id}}); err == nil {
