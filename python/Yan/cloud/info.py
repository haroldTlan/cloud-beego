...
#        self._ifaces = network.ifaces().values()
    ...
    def _stat_weed_cpu(self):
        ...
        available = self._format_nr(total - self._weed_cpu)
    ...
    def _stat_weed_mem(self):
        ...
        weed_total = vm.total/1024/1024/1024
	...
	try:
            if 'g' in mem:
                ...
                self._weed_mem = 0
        available = weed_total - self._weed_mem
        weed_used_per =  float('%0.3f'%float(self._weed_mem/weed_total*100))

        return dict(name=weed_name, total=weed_total, available=available, used_per=weed_used_per)
